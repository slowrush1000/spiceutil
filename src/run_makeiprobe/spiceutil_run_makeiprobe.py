...
class Makeiprobe(run.Run):
    ...
    def makeiprobe(self):
        self.get_input().get_log().get_logger().info(f"# makeiprobe start ... {datetime.datetime.now()}")
        ...
        for netname in self.get_input().get_netnames():
            ...
            self.get_input().get_log().get_logger().info(f"probe file   : {probe_filename}")
            #
            probe_file = open(probe_filename, "wt")
            probe_file.write(f"* {version.Version().get_program()} - {version.Version().get_version()}\n")
        ...
        self.get_input().get_log().get_logger().info(f"# make iprobe end ... {datetime.datetime.now()}")
    ...
    def run(self, args=None):
        ...
        self.run_parser()
        self.makeiprobe()
        self.get_input().get_log().get_logger().info(f"# makeiprobe end ... {datetime.datetime.now()}")
