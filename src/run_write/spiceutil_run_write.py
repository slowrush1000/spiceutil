...
class Write(run.Run):
    def __init__(self, t_input=None, t_netlist=None, textwidth=100, add_first_char=False):
        ...
        self.m_filename = ""
        self.m_textwidth = textwidth
        self.m_add_first_char = add_first_char
    ...
    def get_add_first_char(self):
        return self.m_add_first_char

    def run(self):
        file = open(self.m_filename, "wt")
    ...
    def write_netlist(self, file):
        default_top_cell = self.get_netlist().get_cell(netlist.get_k_default_top_cellname(), netlist.Type.CELL_CELL)
    ...
    def get_inst_line_vcvs(self, instname, nodenames, inst):
        ...
        if netlist.get_k_default_cellname_vcvs() in inst.get_equation_value_dic():
            equation_value = inst.get_equation_value_dic()[netlist.get_k_default_cellname_vcvs()]
            inst_line += f" '{equation_value.get_equation()}'"
        # ...
        for param_name in inst.get_param_names():
